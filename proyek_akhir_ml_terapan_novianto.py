# -*- coding: utf-8 -*-
"""Final Proyek akhir ML Terapan Novianto.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1OvVxW1pOL1dnBpfXAsILax5ri6bU580y
...
### 2.3.4. Evaluasi model content based

**Evaluasi dengan precision**
"""

def precision_at_k(recommended_list, relevant_list, k=5):
    """
    Menghitung precision@k.

    Args:
        recommended_list: List makanan yang direkomendasikan.
        relevant_list: List makanan yang relevan.
    ...
    num_relevant_recommendations = len(set(recommended_list) & set(relevant_list))

    # Hitung precision@k
    precision = num_relevant_recommendations / k

    return precision


# Buat list makanan yang relevan untuk setiap user
relevant_foods_per_user = {}
for name in food_rating['Name'].unique():
    # Use 'name' instead of 'user_id' to access the dictionary
    relevant_foods_per_user[name] = food_rating[(food_rating['Name'] == name) & (food_rating['Rating'] >= 4)]['C_Type'].tolist()


# Buat list makanan yang direkomendasikan untuk setiap user
recommended_foods_per_user = {}
for name in food_rating['Name'].unique():
    # Use 'name' instead of 'user_id' to access the dictionary
    # Pass the food name instead of the C_Type
    recommended_foods_per_user[name] = food_recommendations(name, similarity_data=cosine_sim_df, items=food[['Name', 'C_Type']], k=5)['C_Type'].tolist()
    # Mengambil nama makanan yang direkomendasikan


# Hitung precision@k untuk setiap user
precisions = []
for name in food_rating['Name'].unique(): # Change user_id to name
    precisions.append(precision_at_k(recommended_foods_per_user[name], relevant_foods_per_user[name])) # Change user_id to name


# Hitung rata-rata precision@k untuk semua user
average_precision = np.mean(precisions)

print(f"Precision@k rata-rata: {average_precision}")

"""Hasil dari evaluasi precision menunjukkan nilai 0.16 atau 16%.

**evaluasi dengan recall**

Hasil evaluasi precision yang hanya 16% akan coba dievaluasi ulang dengan recall.

Meningkatkan precision@k sistem rekomendasi ini memerlukan kombinasi praproses data, rekayasa fitur, pemilihan model, penyetelan hiperparameter, dan evaluasi komprehensif.
"""

#  calculating Recall@k

def recall_at_k(recommended_list, relevant_list, k=5):
    """
    Menghitung recall@k.
    ...
    # Hitung recall@k
    recall = num_relevant_recommendations / len(relevant_list) if len(relevant_list) > 0 else 0  # Handle zero division

    return recall


# Use the same relevant_foods_per_user and recommended_foods_per_user from previous code

# Calculate recall@k for each user
recalls = []
for name in food_rating['Name'].unique():
    recalls.append(recall_at_k(recommended_foods_per_user[name], relevant_foods_per_user[name]))

# Calculate average recall@k for all users
average_recall = np.mean(recalls)

print(f"Recall@k rata-rata: {average_recall}")
...
"""### 2.4.7. evaluasi Modeling collaborative
